...
4. 学习算法的原始形式
    ...
    - 采用随机梯度下降法来更新w、b的值，沿着梯度的反方向进行更新
    - 学习步长
5. 算法的收敛性:误分类的次数是有上限的，结果不唯一，需要对分离超平面增加约束条件-->也就是线性支持向量机的想法。
6. 学习算法的对偶形式
    - 将w和b表示成实例xi, yi的线性组合形式，通过求解其系数来求得w和b.
    - 实例点离超平面越近，则越容易误分类。
    - 训练集样本內积：Gram矩阵
...
# 机器学习
## 绪论
定义：假设用P来评估计算机程序在某任务类T上的性能，若一个程序通过利用经验E在
T中任务上获取了性能改善，则我们就说关于T和P，该程序对E进行了学习。

1. 假设空间（hypothesis space）
    - 归纳
    - 演绎
2. 归纳偏好：机器学习算法在学习过程中对某种类型假设的偏好，也可认为对假设
进行选择的启发式。
